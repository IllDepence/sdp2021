...
for i in range(len(ergebnis_tokens)):
    ...
    # returned var is 0 for else, 1 for RU or 2 for UK or 3 for BG
    if language_indicator == 1:
        index_list_ru.append(i)
    if language_indicator == 2:
        index_list_uk.append(i)
    if language_indicator == 3:
        index_list_bg.append(i)   
...
print('Lengths of index lists by language: RU - '+str(len(index_list_ru))+' | UK - '+str(len(index_list_uk))+' | BG - '+str(len(index_list_bg)) +' | Other - '+str(len(index_list_other)))
print('Check: Sum of list lengths: '+str(len(index_list_ru)+len(index_list_uk)+len(index_list_bg)+len(index_list_other)))
...
for train_index, test_index in kf.split(all_pdfs_vectorized_features):
    ...
    for test_item in test_index:
        if test_item in index_list_bg:
            tests_bg_in_index.append(test_item)

    
    x_test = all_pdfs_vectorized_features[tests_bg_in_index]
    y_test = labels_categorized[tests_bg_in_index]
    ...
    if x_test.shape[0]!=0:
        ...
        report_name = 'Classification_Report_BG_'+model_name+'_.csv'
        pd.DataFrame(report).to_csv('results/'+report_name)
    else:
        print('Run #'+str(run)+' SKIPPED - no BG data in test subset')
