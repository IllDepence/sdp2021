...
"""
This file evaluates BiLSTM model instances on all items in Ukrainian language. 
...
for train_index, test_index in kf.split(all_pdfs_vectorized_features):
    ...
    # use only items of test set that are of Ukrainian language 
    tests_uk_in_index = []
    for test_item in test_index:
        if test_item in index_list_uk:
            tests_uk_in_index.append(test_item)

    
    x_test = all_pdfs_vectorized_features[tests_uk_in_index]
    y_test = labels_categorized[tests_uk_in_index]
    ...
    report_name = 'Classification_Report_UK_'+model_name+'_.csv'
