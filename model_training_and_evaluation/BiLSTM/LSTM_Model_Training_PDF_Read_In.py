...
for train_index, test_index in kf.split(all_pdfs_vectorized_features):
    ...
    model.add(Bidirectional(
        LSTM(100, input_shape=(1000, 105), return_sequences=True)))
    ...
    model.compile(loss='categorical_crossentropy', optimizer=opt,
                  metrics=[km.Recall()], sample_weight_mode='temporal')
    print('Compilation successful.')

    # Fit the model in XY epochs (see variable above)

    model.fit(x_train, y_train, batch_size=32, epochs=epoch_number, verbose=1,
              validation_split=0.1, shuffle=True, sample_weight=sample_weights, callbacks=[csv_logger])

    time_finished_training_run = datetime.now()
    time_run_delta = time_finished_training_run-time_started_training_run
    print('Duration of training run #'+str(run)+' : '+"%.2f" %
          (time_run_delta.total_seconds() / 60.0)+' min \n')

    print('Recall score on test data: ' +
          str(model.evaluate(x_test, y_test, verbose=1)[1]))
    scores.append(model.evaluate(x_test, y_test, verbose=1)[1])

    # save current model as file
    model_name = 'Adam-Epochs-' + \
        str(epoch_number)+'-5Fold-Run-FastText-Detection-'+str(run)+'.h5'
